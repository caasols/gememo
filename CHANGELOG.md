...
Gememo started as a single-file proof-of-concept that could leave a Google Meet call and save a Craft note. It grew a full prompt pipeline (built-in meeting templates, time/day rules, per-rule summary depth, recurring-meeting context, per-rule title templates, auto-tags), multiple output destinations (Craft, Apple Notes, Obsidian, Bear, Google Docs) with a multi-destination repeater, local Markdown + YAML backups with auto-cleanup, a History tab with per-meeting terminal save-states and one-click recovery, and resilient capture (delegation-robust Leave interception + a capture-on-meeting-end safety net). It then went through a deliberate **leanness pass** — parking a swathe of beta features (Google Calendar, glossary, webhooks, wikilinks, `.ics`, redaction, note language, review-before-send) and retiring the Experimental toggle entirely — to ship a focused core. Quality: CI on every PR, **~97% native-host coverage**, a **measured/guarded pure-JS layer** (`constants.js` ~99%), an extension E2E harness that loads the real unpacked extension headlessly, a fake-Meet content-script E2E, and an in-popup unit suite. The sections below trace that arc.
...
### Added
- **History now flags meetings that ended without saving, with one-click recovery.** Each meeting in History gets a definitive terminal save-state — **saved** (green, all destinations), **partial** (amber), **failed** (red) — and a meeting that ended having produced only snapshots (never reached a save) now shows as **"Not saved"** with a **Save now** button *and* a proactive recovery card, instead of an ambiguous grey dot. "Save now" reuses the host's snapshot recovery to route the latest snapshot to all your outputs → the meeting flips green. It's the safety net for the residual RES-1 lifecycle cases (tab closed / browser quit before the auto-capture runs) and a way to retroactively rescue a meeting. The dot is also now **honest** — a partial save (one app failed) no longer reads as a full green; and a partial/"no notes" warning is never mistaken for "unsaved" (so recovery can't duplicate a note that already saved). (Extension reload only; host version bumped for lockstep.)

### Fixed
- **Meeting notes are now captured even when the Leave-button click is missed (RES-1) — a real data-loss fix.** On a long multi-person Meet the controls toolbar re-rendered, the Leave-button click listener (bound to a single DOM node) went stale, the user's Leave click was never intercepted, and only periodic snapshots survived — the note never reached any app. Two layers, defense-in-depth: **(1)** leave interception is now **document-level event delegation** (`e.target.closest('button[aria-label="Leave call"]')`), so a toolbar re-render that swaps the button node can't orphan the handler; **(2)** a **capture-on-meeting-end safety net** — when the Leave button vanishes (host-ended call, network drop, or a missed click), after a 2.5 s debounce (to ignore transient re-renders) it routes the latest snapshot to your outputs via the existing proactive-capture path. Idempotent with the normal Leave-click and Gemini-deactivation captures (no double-send). (Extension reload only; host version bumped for lockstep.)

### Changed
- **"Developer logs" → "Developer mode", and the raw activity-log download now hides behind it.** The activity-log download is a debug-only artifact (operational telemetry — saves/failures/retries/recoveries, *not* your meeting notes), so it no longer sits permanently in Settings. The renamed **Developer mode** toggle now gates *both* the verbose History debug rows **and** the **Download activity log** row — with it off, neither shows. **Copy diagnostics report** (the lightweight version/host/settings tool, useful for any bug report) stays always available. (Extension reload only; host version bumped for lockstep.)
...
### Removed
- **Retired the Experimental/Beta toggle entirely.** Parked its last remaining feature — the **Selector hotfix** (RB-1b, the hosted-`selectors.json` resilience valve) — then removed the whole subsystem: the `#beta-enabled` toggle, the **Beta tab** + panel, the `body.beta-enabled` CSS gate, `mm2c_beta_enabled`, and the `'beta'` tab wiring. Nothing was gated behind it anymore. The bundled `SELECTORS` registry and all capture/save are untouched. Selector hotfix is preserved locally and is the **top "revisit next"** item — to come back as a normal Settings feature. (Extension reload only; host version bumped for lockstep.)
- **Parked the last six beta features**, trimming Experimental down to ~just the Selector hotfix: **Webhook** (generic + Slack), **Redaction & blocklist** (PII/keyword redaction + the capture-blocklist), **Wikilinks for graph apps**, **.ics for Next Steps**, **Note language**, and **Review / review-before-send**. Removed end-to-end across the extension *and* the native host (the redaction/wikilinks/.ics/webhook pipeline steps + their builders). The core capture/save pipeline is untouched — notes route to all outputs as before, just without those optional transforms; capture now sends directly (no review overlay). The host's `action_items`/`next_steps` parsing stays (note frontmatter), and `parse_note_sections` is kept for the recurring-meeting prior-context feature. Code preserved locally + re-appliable (one patch). (Host + extension; reload the extension.)
- **Parked the Glossary (custom vocabulary, RB-4a).** The beta-gated Rules-tab textarea that injected a "spell these names/terms exactly" instruction into the Gemini prompt. Removed end-to-end — `glossaryPrefix`, the `glossary` parameter of `assemblePrompt`, the `#glossary` widget + wiring, and its tests. (The core `assemblePrompt` and all other prompt parts are untouched.) Code preserved locally + re-appliable. (Extension reload only; host version bumped for lockstep.)
- **Parked the Today-tab beta widgets: "Email this note" + the "Action items" card.** Both were beta-gated; removed to declutter the Today tab. The action-items card pulled its coupled **Send to tasks** export (the Settings task-app: Things/Todoist/OmniFocus) and the **"Your name"** aliases — which only fed its "N for you" badge — out with it, plus their now-dead helpers. **Kept:** ".ics for Next Steps" and the host's `action_items` extraction (note frontmatter + the .ics). Code preserved locally + re-appliable. (Extension reload only; host version bumped for lockstep.)
...
### Added
- **Onboarding now has an optional "Connect Google for Docs & Calendar" step.** The first-run Welcome checklist gains a fourth, **optional** item for connecting Google (which unlocks Google Docs + Calendar). It ticks when Google is connected (`mm2c_google_connected`). Groundwork for a streamlined Google connect flow. (Extension reload only; host version bumped for lockstep.)
- **Output destinations that can't receive a note are now greyed out (OUT-1, Phase 1).** Previously you could pick an output app that would silently no-op — Craft when it isn't installed, Google Docs when you haven't connected, Obsidian with no vault — and nothing would be saved, with no error. A new host `destination_status` probe reports per-destination availability (Craft/Bear app installed via bundle id, Google Docs OAuth status, an Obsidian vault resolvable; Apple Notes is always available on macOS), and the **Primary output** picker now **disables unavailable options and shows the reason inline** (e.g. "Bear — Not installed", "Google Docs — Not connected") plus a **warning banner when the currently-selected output can't receive notes** ("⚠️ Craft can't receive notes — Not installed. Pick another output or fix it."). Fails open if the host can't be reached (nothing is blocked). Also dropped the stale "(untested)" tag from Obsidian (now live-verified) and removed the dead "Coming soon" (Notion/Evernote) placeholders. Additional-destinations + inline fix buttons follow in Phase 2. (Host change → re-run `install.sh`.)
- **"Notes paused" nudge when the Meet tab is backgrounded.** Periodic snapshots can only run while the Meet tab is focused (capturing needs the foreground), so working in another tab/app lets your notes go stale — previously with no signal. Now, after the tab's been hidden mid-meeting for half a snapshot interval, the extension's toolbar icon shows an amber **⏸** badge (visible from any tab); when you return to Meet, the badge clears and a toast tells you *"Notes were paused while you were away — capturing the latest now…"* as the catch-up snapshot runs. Pairs with the lost-notes-on-leave fix: it keeps you aware so the always-saved snapshot is also as fresh as possible. (Extension reload only — no host change.)

### Fixed
- **Obsidian note titles with a slash no longer run the words together.** `_obsidian_filename` stripped path separators (`/`, `\`) with no replacement, so a meeting titled "Reigo/Carlos" became `…ReigoCarlos.md` instead of `…Reigo Carlos.md`. Path separators now become a **space**, so the title reads the way Craft shows it; the rest of the unsafe set (`* ? " < > |`, control chars) is still dropped, and ordinary punctuation (`% & : ()`) is still kept. (Host change → re-run `install.sh`.)
...
### Fixed
- **`install.sh` no longer aborts before registering the native host when Craft isn't installed.** The installer detects your Craft space id from Craft's local autocomplete cache so notes land in the right space; that lookup is a `ls …/serverSideQuickSearchAutoComplete_*__en | head -1` glob. With Craft absent (or its cache not yet built) the glob matches nothing, and under the script's `set -euo pipefail` the failed pipeline aborted the **entire install** right after the "Google Calendar libraries installed" line — *before* the manifest was ever written. The result: `install.sh` looked like it succeeded, but `io.gememo.host.json` was never created and the popup stayed stuck on "I'm not set up yet" with the native host unreachable. The optional space-detection lookup is now tolerated (`|| true`) so it can't take the install down; the space id is still detected when Craft *is* present. (Host change → re-run `install.sh`.)
...
- **Unit-tested the OAuth token-refresh path in `gcal.py` / `gdocs.py` (TEST-2).** `_load_creds()`'s refresh-on-expiry branch — expired creds with a `refresh_token` calling `creds.refresh()` + re-saving the token, and a failed refresh disconnecting cleanly — was the one untested path with real user impact (a broken refresh silently disconnects Calendar/Docs mid-use), and was unreachable in tests because the google libs are absent in CI. A new `TestLoadCreds` in each test file injects fake `Credentials`/`Request` (via `patch.object(..., create=True)`, no network, no filesystem) covering six cases per module: valid creds (no refresh), expired + `refresh_token` (refresh + save), expired without a `refresh_token`, a refresh that raises (→ `None`, no save), a malformed token file (→ `None`), and an available-but-missing token (→ `None`). The remaining gcal/gdocs gap (interactive OAuth + live API) is deliberately not unit-tested. +12 tests (host suite 374 → 386). (No host change.)
