...
### Changed
- **Friendlier status on the Today tab.** The green status dot now gently pulses ("alive & listening" heartbeat; respects reduced-motion), the line speaks in Gememo's voice — **"I'm alive and listening for meetings…"** (or "I'm not set up yet — click Set up…") — and the version number is gone from there. The native-host version now lives in **About**, next to the Extension ID.
