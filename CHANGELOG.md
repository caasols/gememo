...
### Added
- **Enable/disable toggles for built-in rule templates.** The built-in templates (Standup, 1:1, Retro, …) now render in the Rules tab like normal rules — each with an on/off toggle (default on) — instead of a read-only list. A disabled template is skipped during prompt matching and falls through to the default. New pure `enabledBuiltIns(builtins, disabled)` helper + `mm2c_builtin_disabled` storage key (a list of disabled template names); `content_meet.js` filters `BUILT_IN_RULES` through it before matching. (`findPromptRule` already skipped `enabled:false`.) +8 pure tests + a popup E2E. This is the safe increment of UXF-9 — the full unified/reorderable rules list is still deferred.
