...
### Added
- **Output destinations that can't receive a note are now greyed out (OUT-1, Phase 1).** Previously you could pick an output app that would silently no-op — Craft when it isn't installed, Google Docs when you haven't connected, Obsidian with no vault — and nothing would be saved, with no error. A new host `destination_status` probe reports per-destination availability (Craft/Bear app installed via bundle id, Google Docs OAuth status, an Obsidian vault resolvable; Apple Notes is always available on macOS), and the **Primary output** picker now **disables unavailable options with a "why" tooltip** and shows a **warning banner when the currently-selected output can't receive notes** ("⚠️ Craft can't receive notes — Craft isn't installed. Pick another output or fix it."). Fails open if the host can't be reached (nothing is blocked). Also dropped the stale "(untested)" tag from Obsidian (now live-verified). Additional-destinations + inline fix buttons follow in Phase 2. (Host change → re-run `install.sh`.)
