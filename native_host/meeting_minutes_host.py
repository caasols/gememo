...
HOST_VERSION = '0.3.16'  # in lockstep with manifest.json (major stays 0 → re-run install.sh only to refresh the shown version; not required for compatibility)
...
def _dispatch() -> None:
    ...
    if msg.get("type") == "google_connect":
        # Fail FAST when the flow can't possibly succeed — Google libs missing, or the
        # shared OAuth client wasn't fetched (install.sh pulls credentials.json from
        # the release). Otherwise the doomed detached process exits silently and the
        # popup polls a never-connecting status forever ("Connecting…" with no error)
        # — BUG-14.
        ...
        if not gauth.CREDENTIALS_PATH.exists():
            send_message({"status": "error",
                          "error": "Google client missing — re-run install.sh (it fetches the shared client)."})
            return
