...
class TestBuildCreatedocumentUrl(unittest.TestCase):
    ...
    def test_large_note_encoded_inline_without_truncation(self):
        """A large note (~25 KB) is carried inline in the URL in full.

        createdocument encodes content INLINE (no file staging), so this guards
        that the committed path handles real-world note sizes without our code
        silently dropping/truncating content. (An OS-level craftdocs:// URL
        length ceiling, if any, is separate and outside this builder's control.)
        """
        body = "## Summary\n\n" + ("Carlos confirmed the Q3 roadmap is on track. " * 600)
        self.assertGreater(len(body), 25_000)
        url = build_createdocument_url("Big meeting", body)
        self.assertTrue(url.startswith("craftdocs://createdocument?"))
        # No % in body → a single decode recovers the full content unchanged.
        self.assertEqual(urllib.parse.unquote(self._extract_content(url)), body)
