...
| **Google Docs output** | 🧪 under testing | Selectable as your **Primary output** (Settings → Primary output → Google Docs) — files each captured note as a Google Doc. A **separate** OAuth connect from Calendar (its own `documents` scope + token). Live OAuth + Doc creation not yet maintainer-verified end-to-end. Setup: [`native_host/GDOCS_SETUP.md`](native_host/GDOCS_SETUP.md). |
